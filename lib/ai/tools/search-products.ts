...
const productSearchSchema = z.object({
  query: z
    ...
    .default("")
    .describe(
      "Search term to find producs by name, description, or category (e.g., 'oak table', 'leather sofa', 'dining')",
    ),
  category: z
    .string()
    .optional()
    .default("")
    .describe(
      "Filter by category slug (.eg., 'sofas', 'tables', 'chairs', 'storage')",
    ),
  material: z
  ...
  maxPrice: z
    ...
    .optional()
    .default(0)
    .describe("Maimum price in uSD (e.g., 500). Use 0 for no maximum."),
});

export const searchProductsTool = tool({
  description:
    "Search for products. Can search by name, description, or category, and filter by material, color and price range. Returns product details including stock availability",
  ...
  execute: async ({ query, category, material, color, minPrice, maxPrice }) => {
    ...
    try {
      ...
      console.log("[SearcProducts] Products found:", products.length);
