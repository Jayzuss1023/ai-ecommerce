...
|-----------|------|-------------|
| query | string | Text seatch for product name/description (e.g., "dining table", "sofa") |
| category | string | Category slug: "", "sofas", "tables", "chairs", "storage" |
| material | enum | "", "wood", "metal", "fabric", "leather", "glass" |
| color | enim | "", "black", "white", "oak", "walnut", "grey", "natural" |
...
When user ask for products similar to a specific item (e.g., "Show me products similar to Oak Dining Table"):
...
"To check your orders, you'll need to sign in first. Click the user icon on the top right to sign in or create an account."
`;
...
export function createShoppingAgent({ userId }: ShoppingAgentOptions) {
  ...
  const instructions = isAuthenticated
    ? baseInstructions + orderInstructions
