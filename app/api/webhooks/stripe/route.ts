...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session) {
  ...
  try {
    // Idempotency check: Prevent duplocate processing on webhook retries
    ...
    // Create order in Sanity with custoner reference
    const order = await writeClient.create({
      ...
      clerkUserId,
      email: userEmail ?? session.customer_details ?? "",
      items: orderItems,
    ...
    throw error; // Re-throw to return 500 and trigger Stripe entry
  }
}
